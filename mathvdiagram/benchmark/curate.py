"""
curate.py -- Local curation UI for the concise-prompt set.

Opens a browser with ground truth images + prompts. Check boxes to exclude
rows. Auto-saves excluded_ids.txt so downstream generation/evaluation and the
HuggingFace upload skip low-quality prompts.

Run with::

    python -m mathvdiagram.benchmark.curate
    python -m mathvdiagram.benchmark.curate --category "combinatorics"
    python -m mathvdiagram.benchmark.curate --port 8765
"""
...
CSV_PATH = Path("data") / "concise_prompts.csv"
GT_DIR = Path("data") / "ground_truth"
EXCLUSIONS_PATH = Path("data") / "excluded_ids.txt"
...
def build_html(rows, exclusions, page, per_page, category_filter, total_all):
    total = len(rows)
...
<div class="topbar">
  ...
  <div class="stats">Excluded: <strong id="excl-count">{excluded_count}</strong> / {total_all}</div>
  <div class="actions">
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        html = build_html(self.rows, self.exclusions, page, per_page, self.category_filter, self.total_all)
...
def main():
    ...
    # Total across the whole prompt set (ignoring any category filter).
    Handler.total_all = len(load_rows(None))
    ...
    try:
        ...
        print(f"\nStopped. Exclusions saved to {EXCLUSIONS_PATH}")

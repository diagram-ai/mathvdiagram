...
Reads eval_results.json from each model's output directory and produces:
  1. Summary table (models x metrics) with mean +/- 95% CI
...
Run with::
    ...
    python -m mathvdiagram.benchmark.compare --results-dir outputs
"""
...
def main():
    ...
    # --- Generation stats (read from the same results-dir) ---
    gen_stats = {}
    for model in models:
        log_path = os.path.join(results_dir, model, "generation_log.json")
    ...
    for mk, mname, direction in METRICS:
        for (m1, m2), result in sorted(sig_tests.get(mk, {}).items()):
            ...
            sig_rows.append(
                f"<tr><td>{mname}</td><td>{m1}</td><td>{m2}</td>"
                f"<td>{fv(result['p'], 6)}</td><td>{fv(p, 6)}</td><td>{stars}</td><td>{result['n']}</td></tr>"
            )
    ...
    metric_headers_all = "".join(f"<th>{mn} {'v' if d=='lower' else '^'}</th>" for _, mn, d in METRICS)
...
<p class="subtitle">{len(models)} models &middot; {len(common_ids)} common images</p>

<h2>1. Full Results (All Pairs per Model)</h2>
<p class="note">Each model evaluated on its own successful compilations. Sample sizes differ -- use the common-subset table for fair comparison.</p>
<table>
<thead>
...
<div class="footer">
  <p>Metrics: CMMD (CLIP MMD, unbiased), DISTS (Deep Image Structure &amp; Texture Similarity), CLIP Similarity (CLIP ViT-B/32 cosine), Edge IoU &amp; Edge F1 (Canny edge overlap). Means shown with 95% CI. Significance via Wilcoxon signed-rank with Holm-Bonferroni correction.</p>
</div>

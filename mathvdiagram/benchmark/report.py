"""
report.py -- Per-model HTML report for benchmarking evaluation results.

Produces a single-model report with generation stats, overall metrics,
per-category breakdown, and ground-truth-vs-generated side-by-side images.

Run with::

    python -m mathvdiagram.benchmark.report --model deepseek-v3 --max-images 200
"""
...
def main():
    parser = argparse.ArgumentParser(description="Generate a single-model HTML report")
    parser.add_argument("--model", default="deepseek-v3", help="Model name (matches outputs/<model>/ directory)")
    parser.add_argument("--results-dir", default="outputs", help="Directory containing model output folders")
    ...
    model = args.model
    output_dir = os.path.join(args.results_dir, model)
    ...
    with open(os.path.join(output_dir, "generation_log.json")) as f:
        gen_log = json.load(f)
    ...
    for item in display_images:
        ...
        gen_b64 = img_b64(os.path.join(output_dir, f"{iid}.png"))
        gt_b64 = img_b64(os.path.join(args.gt_dir, f"{iid}.png"))
...
<p class="subtitle">Model: <strong>{model}</strong> &middot; {total_attempted} prompts &middot; {len(per_image)} evaluated</p>
...
<div class="footer">
  <p>Generated by the mathvdiagram benchmarking pipeline &middot; Metrics: CMMD, DISTS, CLIP Similarity (CLIP ViT-B/32 cosine), Edge IoU &amp; Edge F1 (Canny edge overlap). Means shown with 95% CI.</p>
</div>

...
Metrics implemented:
  1. DISTS     -- Deep Image Structure and Texture Similarity (per-pair, lower=better)
  2. CLIP Sim  -- CLIP ViT-B/32 cosine similarity (per-pair, higher=better)
  3. Edge IoU  -- Canny edge-mask intersection over union (per-pair, higher=better)
  4. Edge F1   -- Canny edge-mask F1 / Dice (per-pair, higher=better)
  5. CMMD      -- CLIP Maximum Mean Discrepancy (distribution-level, lower=better)
...
# ---------------------------------------------------------------------------
# Shared CLIP model (used by both CLIP similarity and CMMD)
# ---------------------------------------------------------------------------
...
def _get_clip_model():
    """Return a cached CLIP model and processor."""
...
def compute_dists(gen_path: str, gt_path: str) -> float:
    """DISTS between a generated and ground truth image. Lower = more similar."""
    import piq
    global _dists_model
...
# ---------------------------------------------------------------------------
# Metric 2: CLIP cosine similarity
# ---------------------------------------------------------------------------

def compute_clip_similarity(gen_path: str, gt_path: str) -> float:
    """CLIP ViT-B/32 cosine similarity between two images.
...
# ---------------------------------------------------------------------------
# Metric 5: CMMD (CLIP Maximum Mean Discrepancy)
# ---------------------------------------------------------------------------

class CLIPEmbedder:
    """Extract normalized CLIP embeddings for images (for CMMD)."""

    def __init__(self):
        self.model, self.processor = _get_clip_model()
...
def compute_cmmd(gen_embeddings: np.ndarray, gt_embeddings: np.ndarray) -> float:
    """CMMD between two sets of CLIP embeddings.

    Follows Jayasumana et al. (ICLR 2024): unbiased MMD^2 estimator with an
    RBF kernel and the median heuristic for bandwidth. Lower = more similar.
    """
    ...
    # Unbiased estimator: exclude the diagonal for Kxx and Kyy.
...
# ---------------------------------------------------------------------------
# Metrics 3 & 4: Edge IoU and Edge F1 (edge-based structural overlap)
# ---------------------------------------------------------------------------

def compute_edge_metrics(gen_path: str, gt_path: str) -> dict:
    """Edge IoU and Edge F1 (Dice) using Canny edge masks.
    ...
    try:
        ...
        return {"edge_iou": 0.0, "edge_f1": 0.0, "error": "opencv not installed"}
...
def evaluate_pair(gen_path: str, gt_path: str) -> dict:
    ...
    try:
        results.update(compute_edge_metrics(gen_path, gt_path))
    except Exception as e:
...
def main():
    ...
    for cat, sc in sorted(category_scores.items(), key=lambda x: x[1]["n"], reverse=True):
        print(f"  {cat:30s}  n={sc['n']:4d}  DISTS={fmtv(sc['dists_mean'],3)}  "
              f"CLIP={fmtv(sc['clip_sim_mean'],3)}  EdgeIoU={fmtv(sc['edge_iou_mean'],3)}")

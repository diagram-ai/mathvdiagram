"""
paper_figures.py -- Publication-quality figures, tables, and data exports.

Reads eval_results.json + generation_log.json from each model's output
directory and produces LaTeX tables, PDF figures, and CSV exports.

Run with::

    python -m mathvdiagram.benchmark.paper_figures --outputs-dir outputs --paper-dir paper
    python -m mathvdiagram.benchmark.paper_figures --figures fig1 tab1
"""
...
from .models import MODEL_NAMES, MODEL_ORDER  # noqa: E402
...
def tab1_leaderboard(df, common_df, gen_stats, paper_dir):
    """Generate LaTeX leaderboard tables (both all-pairs and common-subset)."""
    for suffix, data in [("all", df), ("common", common_df)]:
        ...
        caption = (
            "Model performance "
            f"{'(common subset)' if suffix == 'common' else '(all evaluated pairs)'}. "
        ...
        for row in rows:
            cells = [row["model"], str(row["n"]), fmt_cr(row)]
            for mk, _, direction in METRICS:
...
# ---------------------------------------------------------------------------
# Figure 1: Distribution violin plots
# ---------------------------------------------------------------------------
...
def fig2_compile_rates(gen_stats, paper_dir):
    ...
    for i, _m in enumerate(models_present):
...
def fig3_category_heatmap(common_df, paper_dir):
    """Heatmap of CLIP Similarity by category x model."""
    models_present = [m for m in MODEL_ORDER if m in common_df["model"].unique()]
...
def fig5_correlations(df, paper_dir):
    ...
    for i in range(len(metric_keys)):
        for j in range(len(metric_keys)):
            ...
            if i == j:
                ...
                ax.text(0.5, 0.35, f"rho={r_s:.2f}", transform=ax.transAxes, ha="center", fontsize=8, color="#666")
                ax.set_xticks([])
                ax.set_yticks([])
    ...
    fig.suptitle("Metric Correlations (Pearson r, Spearman rho)", fontsize=11, y=1.01)
    plt.tight_layout()
...
def fig6_significance(common_df, paper_dir):
    """Pairwise significance heatmap using Wilcoxon signed-rank (CLIP Sim)."""
    models_present = [m for m in MODEL_ORDER if m in common_df["model"].unique()]
    ...
    sns.heatmap(
        ...
        cbar_kws={"label": "-log10(p) x direction", "shrink": 0.8},
    )
    ax.set_xticklabels(ax.get_xticklabels(), rotation=45, ha="right", fontsize=8)
    ax.set_yticklabels(ax.get_yticklabels(), fontsize=8)
...
def main():
    ...
    parser.add_argument("--outputs-dir", default="outputs", help="Directory with model outputs")
    parser.add_argument("--paper-dir", default="paper", help="Output directory for paper assets")
    parser.add_argument("--figures", nargs="*", default=None, help="Specific outputs to generate (e.g., fig1 tab1)")

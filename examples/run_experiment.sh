...
# Run from the repository root with the venv activated and API keys exported.
#
# Usage:
#   bash examples/run_experiment.sh                              # run all models
#   MODELS="gpt-5.4 deepseek-v3" bash examples/run_experiment.sh  # specific models
#   MODELS="gpt-5.4" WORKERS=10 bash examples/run_experiment.sh   # single model, custom workers
set -uo pipefail
...
for MODEL in $MODELS_TO_RUN; do
    ...
    if ! python -m mathvdiagram.benchmark.generate --model "$MODEL" --workers "$MODEL_WORKERS" 2>&1 | tee -a "$LOGFILE"; then
    ...
    START=$(date +%s)
    if ! python -m mathvdiagram.benchmark.evaluate --model "$MODEL" 2>&1 | tee -a "$LOGFILE"; then
    ...
    python -m mathvdiagram.benchmark.report --model "$MODEL" --max-images 200 2>&1 | tee -a "$LOGFILE" || true
    echo "" | tee -a "$LOGFILE"
...
python -m mathvdiagram.benchmark.compare || echo "WARNING: Comparison report failed"
echo "============================================"
